...
export class SessionRunner {
  ...
  constructor(private readonly deps: RunnerDeps) {
    ...
    this.lifecycle = new TaskLifecycle({
      ...
      attemptRunner: (session, input, acceptedUserId, taskContext) => this.runLoop(session, input, acceptedUserId, taskContext),
  ...
  private async runLoop(
    ...
    const engine = new PermissionEngine(session.id, agentRulesets, session.permission, {
      onAsked: async (request) => {
        ...
        // 而任务 API 会给出一个错的答案。
        await this.lifecycle.markPermissionWait(taskContext?.task.id, request.permission, request.patterns, true);
      },
      onReplied: async (request, reply) => {
        ...
        await this.lifecycle.markPermissionWait(taskContext?.task.id, request.permission, request.patterns, false);
      },
