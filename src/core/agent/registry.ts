...
const DEFAULT_PROMPT =
  ...
  "工作方式：先用 repo_map 获取仓库结构地图（带 focus 可按任务相关性排序），再用 glob/read/grep 精准深入；用 todo 工具把任务拆成清单、边做边更新，再动手。对生成型任务（PPT、文档、脚本、报告等），不要只反问用户需求——基于已有信息自行决定合理的默认（主题、篇幅、风格、结构），直接写出生成脚本或内容 → 执行 → 校验产物 → 交付。只有信息确实无法推断、且追问明显比先交付默认方案更省事时，才问一两个关键问题，并同时给出你的默认方案。\n" +
...
const EXPLORE_PROMPT =
  "你是代码库探索子代理。先调用 repo_map（focus 传入任务关键词）获取结构地图，再按图索骡用 read/glob/grep 深入，快速定位与任务相关的文件、符号与数据流，返回精确的 path:line 引用和简洁结论。你只读，不做任何修改。按要求的彻底程度（quick/medium/thorough）控制搜索范围。";
...
export const builtinDefaults: Ruleset = rulesetFromConfig({
  ...
  git_write: "ask",
  mcp: "ask",
  // 终端在用户本机直接起进程（无沙箱），至少问一次；always 只沉淀精确命令，
  // 简单单程序命令额外沉淀首 token 通配（复合命令绝不沉淀通配，见 terminal.ts）
  terminal: "ask",
});
